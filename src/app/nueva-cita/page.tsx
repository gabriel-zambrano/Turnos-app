  ...
  return (
    <div style={{minHeight:'100vh',background:'#f4f7fb',fontFamily:'DM Sans, sans-serif'}}>
      ...
      <div style={{padding:'1.25rem',maxWidth:500,margin:'0 auto',display:'flex',flexDirection:'column',gap:16}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,maxHeight:200,overflowY:'auto'}}>
              ...
              <button
                  ...
                  style={{
                    ...
                    fontFamily:'DM Sans, sans-serif',transition:'all 0.1s',cursor:'pointer',
                    position:'relative',
                    background: ocupado && hora===h ? '#FAEEDA' : ocupado ? '#f0f0f0' : seleccionado ? '#0f1e2b' : '#f4f7fb',
                    color: ocupado && hora===h ? '#633806' : ocupado ? '#ccc' : seleccionado ? '#fff' : '#555',
                    borderColor: ocupado && hora===h ? '#BA7517' : ocupado ? '#e8e8e8' : seleccionado ? '#0f1e2b' : '#e8e8e8',
                    borderWidth: ocupado && hora===h ? 2 : 0.5,
                    textDecoration: ocupado && hora!==h ? 'line-through' : 'none',
                  }}
                >
                  {h}
                  {ocupado && hora===h && (
                    <span style={{position:'absolute',top:-8,right:-6,background:'#BA7517',color:'#fff',fontSize:9,fontWeight:600,padding:'1px 5px',borderRadius:20,fontFamily:'DM Sans, sans-serif'}}>ST</span>
                  )}
              </button>
                )
              })}
        ...
        {/* Alerta sobreturno */}
      {confirmarSobreturno && (
          <div style={{background:'#FAEEDA',borderRadius:12,padding:'1rem',border:'0.5px solid #EF9F27'}}>
            <div style={{display:'flex',gap:10,alignItems:'center',marginBottom:12}}>
              <div style={{width:36,height:36,borderRadius:'50%',background:'#FAC775',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'#633806',flexShrink:0}}>
                {confirmarSobreturno}
              </div>
              <div>
                <div style={{fontSize:13,fontWeight:600,color:'#412402'}}>Horario ocupado</div>
                <div style={{fontSize:12,color:'#633806'}}>Las {confirmarSobreturno}hs ya tiene un paciente agendado</div>
              </div>
            </div>
            <div style={{display:'flex',gap:8}}>
              <button onClick={()=>setConfirmarSobreturno(null)} style={{flex:1,padding:'0.6rem',borderRadius:10,border:'0.5px solid #e8e8e8',background:'#fff',color:'#666',fontSize:13,fontWeight:500,cursor:'pointer',fontFamily:'DM Sans, sans-serif'}}>
                Cancelar
              </button>
              <button onClick={()=>{setHora(confirmarSobreturno);setEsSobreturno(true);setConfirmarSobreturno(null)}} style={{flex:2,padding:'0.6rem',borderRadius:10,border:'none',background:'#BA7517',color:'#fff',fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans, sans-serif'}}>
        ...
        <button onClick={()=>guardar()} disabled={guardando} style={{width:'100%',padding:'1rem',borderRadius:14,border:'none',background:guardando?'#e5e5e5':esSobreturno?'#BA7517':'#0f1e2b',color:guardando?'#aaa':'#fff',fontWeight:700,fontSize:16,cursor:guardando?'not-allowed':'pointer',fontFamily:'DM Sans, sans-serif',display:'flex',alignItems:'center',justifyContent:'center',gap:8,marginBottom:'2rem'}}>
          {guardando
           ...
           : <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>{esSobreturno ? 'Agendar sobreturno' : 'Agendar turno'}</>
          }
        </button>
      </div>
