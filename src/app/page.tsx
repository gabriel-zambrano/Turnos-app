...
import { Badge, Toast, PageHeader, FilterBar, BtnPrimary, Spinner, MetricCard } from '@/components/UI'
...
export default function Dashboard() {
  ...
  async function enviarMasivo(){
    ...
    const nuevos=citas.filter(c=>c.estado==='pendiente').map(c=>({id:'l'+Date.now()+Math.random(),paciente:c.nombre,canal:'WhatsApp',estado:Math.random()>.15?'enviado':'fallido',hora:new Date().toLocaleTimeString('es-AR',{hour:'2-digit',minute:'2-digit'})}))
    setLogs(p=>[...nuevos,...p])
    setEnviando(false)
    msg(`${nuevos.filter(n=>n.estado==='enviado').length} recordatorios enviados`)
  }
            ...
            <div>
              ...
              {loading?<Spinner/>:(
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  {lista.map(c=>{
                    ...
                    return(
                      <div key={c.id} style={{background:'#fff',border:'0.5px solid #e5e5e5',borderRadius:12,padding:'0.85rem 1rem',display:'flex',alignItems:'center',gap:14}}>
                        ...
                        {c.estado==='pendiente'&&(
                          <button onClick={()=>confirmar(c.id)} style={{fontSize:11,padding:'4px 10px',borderRadius:7,border:'0.5px solid #1D9E75',background:'#E1F5EE',color:'#085041',cursor:'pointer',fontWeight:500,fontFamily:'DM Sans, sans-serif'}}>
                            Confirmar
                          </button>
                        )}
                      </div>
              ...
              <button onClick={enviarMasivo} disabled={enviando||pend===0} style={{marginTop:16,width:'100%',padding:'0.8rem',borderRadius:12,border:'none',background:enviando||pend===0?'#e5e5e5':'#0f1e2b',color:enviando||pend===0?'#aaa':'#fff',fontWeight:600,fontSize:14,display:'flex',alignItems:'center',justifyContent:'center',gap:8,cursor:enviando||pend===0?'not-allowed':'pointer',fontFamily:'DM Sans, sans-serif'}}>
                {enviando
                  ...
                  :<><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Enviar recordatorios ({pend} pendientes)</>
                }
              </button>
            </div>
